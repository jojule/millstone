  ...
  <xsl:variable name="class">datefield<xsl:if test="string-length(../@style) &gt; 0">-<xsl:value-of select="../@style"/></xsl:if></xsl:variable>
      ...
      - <SELECT ID="{@id}" 
        ...
        <xsl:if test="$dhtml">
            <xsl:variable name="yearid"><xsl:value-of select="../integer[@name='year']/@id"/></xsl:variable>
            <xsl:variable name="dayid"><xsl:value-of select="../integer[@name='day']/@id"/></xsl:variable>  
            <xsl:attribute name="ONCHANGE">updateCalendar('<xsl:value-of select="$calendarid"/>','<xsl:value-of select="$yearid"/>','<xsl:value-of select="@id"/>','<xsl:value-of select="$dayid"/>','<xsl:value-of select="$weekbegin"/>','<xsl:value-of select="../@immediate"/>');</xsl:attribute>
        </xsl:if>
...
<xsl:template name="calendar-weekdays" mode="datefield-calendar">
  ...
  <TD>
    <xsl:if test="not(../@readonly) and not(../@disabled)">
      <xsl:attribute name="ONCLICK">calSel('<xsl:value-of select="$calendarid"/>','<xsl:value-of select="$dayid"/>',this.id,'<xsl:value-of select="$immediate"/>')</xsl:attribute>
    </xsl:if>
