 ...
 * source that implements the {@link org.millstone.base.data.Property}
 * interface. <code>AbstractField</code> implements that interface itself,
 * too, so accessing the Property value represented by it is
 * straightforward.</p>
 *
 * <p>AbstractField also provides the
 * {@link org.millstone.base.data.Buffered} interface for buffering the data
 * source value. By default the Field is in write through-mode and
 * {@link #setWriteThrough(boolean)} should be called to enable
 * buffering.</p>
 * 
 * <p>The class also supports
 * {@link org.millstone.base.data.Validator validators} to make sure
 * the value contained in the field is valid.</p>
 *
 * @author IT Mill Ltd.
 * @version @VERSION@
 * @since 3.0
...
public abstract class AbstractField
	...
	/* Save the current value to the data source
	 * Don't add a JavaDoc comment here, we use the default documentation from
	 * the implemented interface.
	 */
	...
	/* Test if the field is in write-through mode.
	 * Don't add a JavaDoc comment here, we use the default documentation from
	 * the implemented interface.
	 */
	...
	/* Set the field's write-through mode to the specified status
	 * Don't add a JavaDoc comment here, we use the default documentation from
	 * the implemented interface.
	 */
	...
	/* Test if the field is in read-through mode.
	 * Don't add a JavaDoc comment here, we use the default documentation from
	 * the implemented interface.
	 */
	...
	/* Set the field's read-through mode to the specified status
	 * Don't add a JavaDoc comment here, we use the default documentation from
	 * the implemented interface.
	 */
	...
	/** Gets the current value of the field. This is the visible, modified
	 * and possible invalid value the user have entered to the field. In the
	 * read-through mode, the abstract buffer is also updated and validation
	 * is performed.
	 * 
	...
	/**<p>Sets the specified Property as the data source for the field. All
	 * uncommitted changes to the field are discarded and the value is
	 * refreshed from the new data source.</p>
	 * 
	 * <p>If the datasource has any validators, the same validators are added 
	 * to the field. Because the default behavior of the field is to allow
	 * invalid values, but not to allow committing them, this only adds visual 
	 * error messages to fields and do not allow committing them as long as
	 * the value is invalid. After the value is valid, the error message is not
	 * shown and the commit can be done normally.</p>
	 * 
	...
	/** Adds a new validator for the field's value. All validators added to
	 * a field are checked each time the its value changes.
	 * 
	 ...
	 * the current value is valid, <code>false</code> otherwise
	 */
	...
	public void validate() throws Validator.InvalidValueException {
		...
		Validator.InvalidValueException[] exceptions =
			new Validator.InvalidValueException[errors.size()];
		int index = 0;
	...
	/** Fields allow invalid values by default. In most cases this is wanted, because
	 * the field otherwise visually forget the user input immediately.
	 * 
	...
	/** Fields allow invalid values by default. In most cases this is wanted, because
	 * the field otherwise visually forget the user input immediately. In common setting
	 * where the user wants to assure the correctness of the datasource, but allow
	 * temporarily invalid contents in the field, the user should add the validators
	 * to datasource, that should not allow invalid values. The validators are automatically
	 ...
	 * message thrown by the superclasses (that is the component error message),
	...
	public void addListener(Property.ValueChangeListener listener) {
		addListener(
			AbstractField.ValueChangeEvent.class,
			listener,
			VALUE_CHANGE_METHOD);
	}

	/* Remove a value change listener from the field.
	 * Don't add a JavaDoc comment here, we use the default documentation
	 * from the implemented interface.
	 */
	...
	/** Emit a value change event. The value contained in the field is
	...
	static {
		try {
			READ_ONLY_STATUS_CHANGE_METHOD =
				Property.ReadOnlyStatusChangeListener.class.getDeclaredMethod(
	...
	/* Add a read-only status change listener for the field.
	 * Don't add a JavaDoc comment here, we use the default documentation
	 * from the implemented interface.
	 */
	...
	/* Removes a read-only status change listener from the field.
	 * Don't add a JavaDoc comment here, we use the default documentation
	 * from the implemented interface.
	 */
	...
	/** Emit a read-only status change event. The value contained in the
	 ...
	 * changes forwards.
	 * 
	 * @param event the value change event telling the data source contents
	 * have changed
	 */
